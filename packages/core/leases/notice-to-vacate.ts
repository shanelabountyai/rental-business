...
export interface RentIncreaseNoticeContext {
  ...
  fromCents: number
  toCents: number
  /// The first day the new rent applies - a calendar day, same reason as
  /// `NonRenewalNoticeContext.effectiveOn`.
  effectiveOn: BusinessDate
  /// JurisdictionRule.rentIncreaseNoticeDays, stated when configured.
  rentIncreaseNoticeDays: number | null
}

/**
 * The written notice that makes a rent increase effective (LEASE-09, R-225).
 * A DRAFT, same posture as `nonRenewalNoticeText`.
 */
export function rentIncreaseNoticeText(context: RentIncreaseNoticeContext): string {
  return [
    'Notice of rent increase',
    '',
    `Dear ${context.tenantName},`,
    '',
    `This is written notice that the monthly rent for ${context.addressLine1}${
      context.unitName ? ` (${context.unitName})` : ''
    } will change from ${formatCents(context.fromCents)} to ${formatCents(context.toCents)}, starting ${formatDate(context.effectiveOn)}.`,
    '',
    'Until then your rent stays as it is. All other terms of your tenancy are unchanged.',
    context.rentIncreaseNoticeDays != null
      ? `Your jurisdiction requires at least ${context.rentIncreaseNoticeDays} days' written notice of a rent increase.`
      : null,
    '',
