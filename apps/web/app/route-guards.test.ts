...
const PUBLIC_ROUTES: Record<string, string> = {
  ...
  'verify/[token]/photos/[documentId]/route.ts':
    "Bytes for the verify page above, authorized by the same token in the same path position (R-142) - the tenant's twin of vendor/[token]/documents/[documentId] and separate from api/documents/[id]/file for the identical reason: that route authorizes two kinds of SESSION, and a bearer token falling through to either is the shape R-018's lesson warns about. A tenant answering 'was this fixed?' has no session BY CONSTRUCTION, which is the whole point of the verify link. Its scope check is NARROWER than the vendor route's: exactly the COMPLETION_PHOTOs on exactly that work order, where a vendor may also read the ticket's photos of the problem they were sent to fix. Refuses with 404 for a bad token, for a document on another job, and for any other document type - never 403, so a status code cannot confirm a guessed id is real.",
  'clarify/[token]/page.tsx':
    "The zero-login CLARIFY page (MAINT-01, MAINT-02, R-177). Same shape as verify/[token] above - the token in the path IS the credential and verifyClarifyLink() is the authorization - and it exists for the same persona and the same reason: a request that arrived by TEXT opens with no category, no clarifying prompts, no troubleshooting script and no photo, and linking into /portal/maintenance/... would hit requireTenant, which redirects to an EMAIL-ONLY login with no return-to. It is scoped to one Ticket and, in the token metadata rather than re-derived at use time, one Tenant; it opens no session, reads no document and moves no money. Its business gate is NARROWER than the verify link's: it refuses once a work order exists on that ticket or the status has left NEW/TRIAGED/WAITING_ON_TENANT, because past that point the description is the sheet somebody is being sent out with. SINGLE-USE, burned on submit rather than on open (PROSPECT_PRESCREEN's shape), which is both the 'you have already answered' state and the guard against two taps of Send appending the same transcript twice. The write never overwrites a category a PM chose during triage, never walks back a triaged priority, and only ever RAISES habitabilityFlag.",
  'pay/[token]/page.tsx':
  ...
  'api/email/inbound/route.ts':
    "The inbound-email webhook (COMM-08, R-097a), the sibling of api/sms/inbound above and arranged the same way. A provider has no session and no credentials of ours to present, so a shared secret - compared in constant time, from a header or the URL - is the whole authentication story, and it refuses outright when INBOUND_EMAIL_SECRET is unset, the same posture the SMS and cron routes take. WEAKER THAN TWILIO'S SIGNATURE, and deliberately so: inbound-email providers vary and most offer only a secret. The mitigation is what the endpoint can DO - it files a message into an existing conversation or into the unrouted triage queue, and cannot create a tenancy, move money or say who anybody is. Routing itself never trusts the sender: `decideRoute` refuses to guess between candidates exactly as it does for SMS, and a From: header is only as trustworthy as caller ID.",
  ...
  'showings/access/[token]/page.tsx':
    "A prospect's link to the door for a self-showing they have already booked (LEASE-08, R-094), answerable with no account - a prospect has none. The token in the path IS the credential and `showingAccessLinkStatus()` is the entire authorization. MULTI-USE unlike its booking sibling, because it is opened at least twice by design - once in advance to get the identity check done, once standing at the door - and a link that died on first open would leave somebody outside a house. THE TOKEN IS NOT WHAT PROTECTS THE CODE: `selfShowingDecision()` re-decides on every render from the unit's status, the showing's status, the identity check and `ShowingAccess.revokedAt`, so a leaked link outside the booked window shows a sentence rather than a code, and a killed code stops appearing on the next refresh.",
  'api/calendar/[token]/route.ts':
    "A staff member's own read-only visit calendar (NOTIF-06, R-097c), fetched by a CALENDAR APP with no session and nobody watching. The token in the path is the entire credential and `checkToken` is the entire authorization, the same shape sign/[token]/document/route.ts already uses for bytes. THE SCOPE IS NOT IN THE TOKEN: the route loads the staff actor and resolves `currentScope` fresh on every fetch, so a person moved to one property or deactivated stops seeing the rest on their calendar's next poll - which is what ROLE-06's 'access dies within a minute' means for a surface nobody logs out of. It answers 404 for every bad token rather than a message, because there is nobody to read one and a distinguishable 'expired' would confirm a token existed. `mfaVerified` is always false: a link sitting in a calendar app is not a proved second factor."
}
...
const RESOURCE_LESS_GUARDS: Record<string, string> = {
  ...
  'app/(admin)/jobs/page.tsx':
    'A JobRun belongs to a property, but "did last night\'s work happen" does not (R-174): the panel answers a question about the whole deployment - every job, every property, and the jobs that have never run anywhere. Half that answer is worse than none, so refusing a scoped actor is the intended result.',
  'lib/jobs/actions.ts':
    'Same as the job-health panel above. The re-run is pressed FROM that screen. The manager role carries `job.manage` since R-197, so this refuses a property-scoped manager and admits a portfolio-wide one - the same line the panel draws.',
